...
function baseResults(): EmissionResults {
  return {
...
export function calculateEmissions(farm: FarmData): EmissionResults {
  ...
  for (let i = 0; i < farm.num_crops; i += 1) {
    ...
    const livestockEmissions =
      totalCropArea > 0 ? results.livestock_emissions * (crop.area / totalCropArea) : 0.0;
    ...
    const cropResult: CropEmissionResults = {
...
export function calculateLegacyEmissions(farm: LegacyFarmData): EmissionResults {
  const results = baseResults();

  const nitrogenEmissions = (farm.nitrogen_kg_ha * farm.farm_size * NITROGEN_FACTOR) / 1000.0;
  const phosphorusEmissions =
    (farm.phosphorus_kg_ha * farm.farm_size * PHOSPHORUS_FACTOR) / 1000.0;
